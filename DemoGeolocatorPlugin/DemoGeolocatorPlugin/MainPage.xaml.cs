...
namespace DemoGeolocatorPlugin {
        ...
        public MainPage() {
            ...
            buttonGetGPS.Clicked += async (sender, args) => {
                try {
                    ...
                    var position = await locator.GetPositionAsync(new TimeSpan(0,0,1));
            ...
            buttonTrack.Clicked += async (object sender, EventArgs e) => {
                try {
                    if (CrossGeolocator.Current.IsListening) {
                        ...
                        if (await CrossGeolocator.Current.StartListeningAsync(new TimeSpan(0,0,1), 0)) {
